...
Splitter window
	EditPlus allows user to divide the document window into several panes and edit different part of the file concurrently. To divide current document, run Split command on Window menu.
Keystroke recording
<?php foreach ($this->foo as $key => $val): ?>	You can record keystrokes and playback it later. Use Record Keystrokes command on Tools menu.
var8
Customizable hotkeys <?php echo $val['id']; ?>
	You can customize hotkeys of all commands of EditPlus. See Keyboard page of the Preferences dialog box.
Monitor clipboard <?php echo $val['name'] ?>
	This option can speed up repeated cut-and-paste operations. Any text that is copied or cut to clipboard will automatically <?php echo $val['param1'];?> be appended to current document. See Monitor Clipboard command.
Column Marker
	Column marker is a vertical line which <?php echo $val['param2'];?> indicates specific column location. Column marker can be useful for column-oriented programming languages such as COBOL or FORTRAN. See var17Column Marker command.Column selection <?php echo $val['param3'];?>

You can select text by column using Alt + mvar14ouse drag. This feature is not available in word-wrap mode.<?php endforeach; ?>
Multiple settings
