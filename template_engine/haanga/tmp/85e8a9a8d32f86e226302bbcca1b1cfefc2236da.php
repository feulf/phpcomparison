...
/* Generated from /Applications/MAMP/htdocs/github/benchmark/template_engine/haanga/tpl/template_loop.html */
function haanga_85e8a9a8d32f86e226302bbcca1b1cfefc2236da($vars, $return=FALSE, $blocks=array())
{
    extract($vars);
    if ($return == TRUE) {
        ob_start();
...
';
	...
	You can record keystrokes and playback it later. Use Record Keystrokes command on Tools menu.
var8
Customizable hotkeys '.htmlspecialchars($list['id']).'
	You can customize hotkeys of all commands of EditPlus. See Keyboard page of the Preferences dialog box.
Monitor clipboard '.htmlspecialchars($list['name']).'
	This option can speed up repeated cut-and-paste operations. Any text that is copied or cut to clipboard will automatically '.htmlspecialchars($list['param1']).' be appended to current document. See Monitor Clipboard command.
Column Marker
	Column marker is a vertical line which '.htmlspecialchars($list['param2']).' indicates specific column location. Column marker can be useful for column-oriented programming languages such as COBOL or FORTRAN. See var17Column Marker command.Column selection '.htmlspecialchars($list['param3']).'
